...
for k, v in [("page","dashboard"),("sel_line",None),("wo_action",None),("edit_record_id",None),("form_shift","DS")]:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if not line_recs:
        st.info("No records found for this line.")
    else:
        dates = sorted(set(r.get("date","") for r in line_recs), reverse=True)
        for d in dates:
            day_recs = [r for r in line_recs if r.get("date")==d]
            with st.expander(f"📅 {d}  —  {len(day_recs)} record(s)", expanded=(d==str(date.today()))):
                for r in sorted(day_recs, key=lambda x: x.get("time","")):
                    ...
                    with c3: st.markdown(f"WO: {r.get('wo','?')} | {r.get('operator','?')}{edited}")
                    with c4:
                        if st.button("✏️ Edit", key=f"edit_{r.get('id','')}", use_container_width=True):
                            st.session_state.edit_record_id=r.get("id",""); st.session_state.sel_line=line; st.session_state.page="form_wcfqc05"; st.rerun()
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.divider()
    color_defaults={1:["WHITE"],2:["FIELD GREEN","OLIVE GREEN"],3:["FIELD GREEN","APPLE GREEN","OLIVE GREEN"],4:["FIELD GREEN","APPLE GREEN","OLIVE GREEN","LIME GREEN"]}
    with st.form("wo_form"):
        ...
        with c2:
            ...
            n_colors=st.selectbox("Number of Colors *",[1,2,3,4],index=(existing["color_count"]-1) if existing else 2)
        st.markdown("**Color Names**")
        prev_colors=existing.get("colors",[]) if existing else []
        ccols=st.columns(n_colors); colors_in=[]
        for idx in range(n_colors):
            default=prev_colors[idx] if idx<len(prev_colors) else color_defaults[n_colors][idx]
            with ccols[idx]: colors_in.append(st.text_input(f"Color {idx+1}",value=default,key=f"ci_{idx}"))
        if st.form_submit_button("💾 Save Work Order",use_container_width=True,type="primary"):
            if not wo_num or not item_cd: st.error("WO Number and Item Code are required!")
                ...
                db["work_orders"][line]={"wo_number":wo_num,"item_code":item_cd,"item_name":item_nm,"color_count":n_colors,"colors":[c.upper() for c in colors_in if c],"created_at":datetime.now().isoformat()}
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    for form in FORMS:
        ...
        with c3:
            ...
            if st.button("▶ Open",key=f"f_{form['code']}",use_container_width=True,type="primary" if impl else "secondary",disabled=not impl):
                st.session_state.edit_record_id=None; st.session_state.page=f"form_{form['code'].replace('-','').lower()}"; st.rerun()
        st.divider()
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if st.button(f"← Back to {'Records' if is_edit else 'Forms'}"): st.session_state.page=back_page; st.rerun()

    if not wo and not existing_rec: st.error("No active WO!"); st.stop()
    if not wo and existing_rec:
        wo={"wo_number":existing_rec.get("wo",""),"item_code":existing_rec.get("item",""),"item_name":existing_rec.get("item_name",""),"color_count":len(existing_rec.get("colors",[])),"colors":existing_rec.get("colors",[])}

    colors=wo.get("colors",[]); color_count=wo["color_count"]

    if is_edit:
        st.markdown(f'<div class="edit-banner">✏️ EDITING RECORD — {existing_rec.get("date","")} | {existing_rec.get("shift","")} | {existing_rec.get("time","")} | Saved by: {existing_rec.get("operator","")}</div>', unsafe_allow_html=True)
    ...
    if st.button(btn_label,use_container_width=True,type="primary"):
        ...
        record={"form":"WC-F-QC-05","saved_at":datetime.now().isoformat(),"line":line_v,"shift":shift_v,"date":str(date_v),"time":time_v,"wo":wo["wo_number"],"item":wo["item_code"],"item_name":wo.get("item_name",""),"operator":opr_v,"verified_by":verified_v,"colors":colors,"positions":positions,"test_data":all_data,"sci":sci_data,"spool_no":spool_no}
        if is_edit:
